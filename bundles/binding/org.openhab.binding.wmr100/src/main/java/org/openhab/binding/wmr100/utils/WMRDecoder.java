 ...
 *
 */
public class WMRDecoder {
	
	private static final Logger logger = LoggerFactory.getLogger(WMRDecoder.class);
	...
	private static int logFlags = LOG_HOUR | LOG_FRAME | LOG_SENSOR | LOG_USB;
	...
	private static void analyseAnemometer(byte[] frame) {
		...
		// JCO 
		WMRDecoder.DATA.put("windDirection", windDirection);
		WMRDecoder.DATA.put("windAVerage", windAverage);
		WMRDecoder.DATA.put("windChill", windChill);
	}
	...
	private static void analyseBarometer(byte[] frame) {
		...
		// JCO
		DATA.put("pressureAbsolute", pressureAbsolute);
		DATA.put("pressureRelative", pressureRelative);
		DATA.put("weatherForecast", weatherForecast);
		DATA.put("weatherPrevious", weatherPrevious);
	}
	 ...
	 *             -output exception
	 */
	public static void analyseFrame(byte[] frame) throws IOException {
	 ...
	 *             -output exception
	 */
	public static void analyseResponse(int bytes, byte[] buffer)
			throws IOException {
	...
	private static void analyseThermohygrometer(byte[] frame) {
		...
		// JCO
		if (sensor == 0) {
			WMRDecoder.DATA.put("indoorTemperature", temperature);
			WMRDecoder.DATA.put("indoorHumidity", humidity);
			WMRDecoder.DATA.put("indoorDewPoint", dewpoint);
		}
	}
...
//				+ DATE_FORMAT.format(new Date()) + "] " + message;
	...
	private static void stationRead() throws IOException {
		while (true) { // loop indefinitely
			System.out.println("STATION READ");
			...
			System.out.printf("received %s bytes\n", responseBytes);
	...
	private static void stationRequest() throws IOException {
		System.out.println("STATION REQUEST");
		if (logFlags != 0 && logFlags != LOG_HOUR) // not just hourly data?
